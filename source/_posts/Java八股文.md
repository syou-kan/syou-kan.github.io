---
title: java面试
headimg: https://static.dearxuan.com/background/pc/1.jpg
categories:
- 面试
tag:
- 面试八股文
...
1. 装载：装载阶段指的是将class文件的字节码加载到内存中，并且生成对应的Class对象。加载class文件有三种方式：

- 从本地文件系统加载：即通过Java虚拟机的类加载器从本地文件系统中加载class文件。
- 从JAR包加载：将JAR包中的class文件加载到内存中。
- 通过网络加载：通过网络下载class文件，并加载到内存中。
...
2. 假如这个类存在直接父类，并且这个类还没有被初始化（注意：在一个类加载器中，类只能初始化一次），那就初始化直接的父类（不适用于接口）
   1. 加入类中存在初始化语句（如static变量和static块），那就依次执行这些初始化语句。	
