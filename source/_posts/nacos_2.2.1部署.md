---
title: nacos_2.2.1部署
headimg: https://static.dearxuan.com/background/pc/3.jpg
categories:
- 搭建
tag:
- nacos
...
> 将`nacos`持久化到`mysql`数据库中
>
> 新建`nacos`数据库

### 从[[nacos-mysql.sql](https://github.com/alibaba/nacos/blob/develop/distribution/conf/nacos-mysql.sql?spm=a2c6h.12873639.article-detail.9.b10d6b47wBGQdH&file=nacos-mysql.sql)]下载建表语句。也可以将下列语句粘贴执行
...
```shell
docker cp nacos:/home/nacos/conf/application.properties /mydata/nacos/config/
```

2. ##### 拷贝`logback`日志配置文件

```shell
docker cp nacos:/home/nacos/conf/nacos-logback.xml /mydata/nacos/config/
```
...
# 修改此行,添加MYSQL_SERVICE_PASSWORD的默认值为root(自己root用户的密码)
...
services:
  nacos:
    ...
    environment:
      ...
      - JVM_MS=64m
      - JVM_MMS=64m
      - MODE=standalone # 使用 standalone模式（单机模式）
      - NACOS_REPLICAS=1
...
# 将用户信息转为 JSON 字符串并进行 URL 编码
$jsonString = $userInfo | ConvertTo-Json
$encodedJson = [System.Web.HttpUtility]::UrlEncode($jsonString)

# 将编码后的字符串转为 Base64 字符串
$base64String = [Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes($encodedJson))
